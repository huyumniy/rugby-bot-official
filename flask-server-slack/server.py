...
slack_token = "xoxb-773919780944-5538333743655-GoHXykrRMBRDyFsE64MlnCPc"
slack_client = WebClient(token=slack_token)
# Create a new book
@app.route('/book', methods=['POST'])
def create_book():
    global counter
    pattern = r'(Resale RESALE - )(.+)( v )(.+)'
    if request.json:
        for el in request.json:
            # Find matches and groups
            matches = re.match(pattern, el["title"])
            if matches:
                country1 = matches.group(2)
                group2 = matches.group(3)
                country2 = matches.group(4)
