...
def check_for_nan(element):
  if not np.isnan(element): return int(element)
  return element
...
def read_excel(file_path):
    ...
    for i in range(len(df)):
        ...
        match_data = {
            "match": match_info[1],
            "categories": {
                1: check_for_nan(match_info[2]),
                2: check_for_nan(match_info[3]),
                3: check_for_nan(match_info[4]),
                4: check_for_nan(match_info[5]),
            },
            "link": match_info[6]
        }
...
def start(link, categories, proxy, email, password):
  ...
  while True:
    ...
    for _ in range(0, categories[category]): check_for_element(driver, '//*[@class="ticket-category-list js-block-list-categories"]//li[not(.//*[contains(text(),"Unavailable")])]//span[@class="operator more active"]', click=True, xpath=True)
